def handle(client, data, secrets):
    ...
    print("Assets Length: ", len(assets_simplified))
    print("Time Series Length: ", len(time_series_simplified))
    # Train the ML Entity Matcher on the data.
    # The SDK expects as input the array of objects you match FROM (time series) and a list of what you match TO (assets)
    t0 = time.time()
    model = client.entity_matching.fit(
        sources=time_series_simplified, targets=assets_simplified
    )
    ...
    # Use the ML Entity Matcher model to match the data.
    # This model can be reused, so training is not necessary each time, but we do it for simplicity in this example.
    t0 = time.time()
    ...
    keep_good_matches_fn = lambda match: match["score"] >= good_match_threshold
    good_match_count = 0
    time_series_updates = []
    for item in result["items"]:
        match_from = item["source"]  # Time series
        matches = item["matches"]  # Suggested asset matches for the time series
        good_matches = list(filter(keep_good_matches_fn, matches))
        if len(good_matches) > 0:
            good_match_count += 1
            best_match = good_matches[0]
            time_series_updates.append(
                TimeSeriesUpdate(id=match_from["id"]).asset_id.set(
                    best_match["target"]["id"]
                )
            )

    dry_run = data.get("update_timeseries", False)
    if not dry_run:
        client.time_series.update(
            time_series_updates
        )  # uncomment to actually update the asset_id field
